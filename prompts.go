...
Если в конце входных данных есть блок "=== MEMORY ===" — это контекст из персистентной памяти о данном отправителе (прошлые взаимодействия, роль, проекты). Учти его при анализе.
...
ПАМЯТЬ:
- Если доступны инструменты memory_search / memory_store — используй их:
  1. ПЕРЕД категоризацией: вызови memory_search для каждого отправителя (по email или имени), чтобы узнать контекст прошлых взаимодействий. Это поможет точнее определить важность и категорию.
  2. ПОСЛЕ категоризации: вызови memory_store для отправителей, о которых появилась новая информация (новый контакт, изменение роли, новый проект). Используй entity_id формата "contact:email@example.com".
- Используй memory_temp_put для промежуточных заметок (например, связь между отправителями или перекрёстные ссылки между письмами).
...
Твоя задача:
1. Сначала вызови memory_temp_get (без ключа) — посмотри, есть ли уже проанализированные темы, которые пересекаются с текущей. Если есть связь — упомяни её в анализе.
2. Загрузи каждую статью через инструмент web_fetch_summarize (prompt: "Извлеки ключевые факты, цифры, цитаты, позиции сторон из этой новостной статьи").
3. Сравни подачу разных источников: что подчёркивает каждый, какие детали добавляет или опускает.
4. Напиши краткий анализ (3-7 предложений): суть события + различия в подаче.
5. После анализа вызови memory_temp_put с ключом вида "topic:краткое-название" и значением — 1-2 предложения с ключевыми фактами. Это поможет следующим темам найти пересечения.
...
// MemoryPromptHint is appended to the system prompt when memory tools are available.
const MemoryPromptHint = `
- You have persistent memory across conversations. IMPORTANT workflow:
  1. BEFORE answering, ALWAYS call memory_search with keywords from the user's message (names, topics, emails, projects). This is cheap and fast — do it proactively even if you are not sure there is relevant context. If the user mentions a person, search for them. If they ask about a topic, search for it. If they continue a previous conversation, search for it.
  2. Use memory_recall to get full details when memory_search finds a relevant entity.
  3. AFTER the conversation, use memory_store to save new information. In particular:
     - USER PROFILE: automatically detect and save facts about the user — location, city, timezone, language, occupation, interests, family, preferences (entity_id: "user:profile"). Update whenever you learn something new.
     - CONTACTS: save info about people the user interacts with — names, roles, relationships, communication preferences.
     - TOPICS: save ongoing topics, project statuses, decisions made.
     - Do not store trivial or one-off information (simple greetings, weather questions, etc).
