...
func runNewsSummary(cfg modelConfig, modelID string, showThinking bool, contentOut io.Writer, logf func(string, ...any), configPath string, prompts *Prompts, mcpMgr *MCPManager, mcpNames []string) (string, error) {
	...
	for ci, cat := range categories {
		...
		for i := range cSources {
			...
			raw, err := doChat(cfg.BaseURL, modelID, messages, cfg.Limit.Output, cfg.Limit.Context)
			if err != nil {
		...
		clusterRaw, err := doChat(cfg.BaseURL, modelID, clusterMessages, cfg.Limit.Output, cfg.Limit.Context)
		if err != nil {
		...
		for _, t := range clustering.Topics {
			...
			analysis, err := doSubAgentWithTools(cfg.BaseURL, modelID, messages, subAgentDefs, cfg.Limit.Output, cfg.Limit.Context, 5, 15000, logf, subAgentExec)
			if err != nil {
